...
//   });
//   state["전문교양"]["필수영역"] =
//     dragonBall["일교4"] || dragonBall["일교5"] ? true : false;
//   if (
//     state["전문교양"]["필수영역"] &&
//     Object.keys(dragonBall).filter((x) => dragonBall[x]).length >= 6
//   ) {
//     state["전문교양"]["드래곤볼"]["qualify"] = true;
//   } else {
//     state["전문교양"]["드래곤볼"]["qualify"] = false;
//   }
...
module.exports = function condition1func(myHistory, myGraduateInfo) {
  ...
  myGraduateInfo.map((x) => {
    switch (x.subject) {
      case "일교1":
        dragonBall["일교1"] = x.degree > 0 ? 1 : 0;
        break;
      case "일교2":
        dragonBall["일교2"] = x.degree > 0 ? 1 : 0;
        break;
      case "일교3":
        dragonBall["일교3"] = x.degree > 0 ? 1 : 0;
        break;
      case "일교4":
        dragonBall["일교4"] = x.degree > 0 ? 1 : 0;
        break;
      case "일교5":
        dragonBall["일교5"] = x.degree > 0 ? 1 : 0;
        break;
      case "핵교6":
        dragonBall["핵교6"] = x.degree > 0 ? 1 : 0;
        break;
      case "핵교7":
        dragonBall["핵교7"] = x.degree > 0 ? 1 : 0;
        break;
      default:
        break;
    }
  ...
  state["전문교양"]["필수영역"] =
    dragonBall["일교4"] || dragonBall["일교5"] ? 1 : 0;
  if (
    state["전문교양"]["필수영역"] &&
    Object.keys(dragonBall).filter((x) => dragonBall[x]).length >= 6
  ) {
